...
public class APITest 
{
    ...
    public void itThrowsExceptionWhenUrlIsInvalid() {
        exceptionRule.expect(RuntimeException.class);
        exceptionRule.expectMessage("Server returned HTTP response code: 403 for URL: https://api.proxycrawl.com?token=testtoken&url=apple");
        API api = new API("testtoken");
        api.get("apple");
    }

    @Test
    public void itThrowsExceptionWhenUrlIsInvalidUsingPost() {
        exceptionRule.expect(RuntimeException.class);
        exceptionRule.expectMessage("Server returned HTTP response code: 403 for URL: https://api.proxycrawl.com?token=testtoken&url=apple");
        API api = new API("testtoken");
        api.post("apple", null);
